        ...
        if entry.tag == 'block':
            ...
            edges.append(
                dict(
                    ...
                    dst=entry.find('to').text,
                    char=entry.find('read').text,
                    write=entry.find('write').text,
                    move=entry.find('move').text,
                )
            )
    assert start_state is not None
    return dict(
...
def parse_zip(zf, pda=False, turin=False):
    if pda:
        ...
        for fname in zf.infolist():
            if not fname.is_dir() and fname.filename.endswith('.jff'):
                root = ET.fromstring(zf.read(fname))
                yield fname.filename, parse_turin(root)
    else:
        for fname in zf.infolist():
            if not fname.is_dir() and fname.filename.endswith('.jff'):
                root = ET.fromstring(zf.read(fname))
...
def load_from_zip(filename, pda=False, turin=False):
    with zipfile.ZipFile(filename, "r") as zf:
        return dict((Path(p).stem, dfa) for p, dfa in parse_zip(zf, pda, turin))


def load_from_list(filenames, pda=False, turin=False):
    result = dict()
    if pda:
        ...
        for filename in filenames:
            result[Path(filename).stem] = parse_turin(
                ET.parse(filename).getroot())
    else:
        for filename in filenames:
...
def main():
    ...
    parser.add_argument('--pda', action='store_true',
                        help='parse from pda')
    # add flag to support turing machine
    parser.add_argument('--tm', action='store_true',
                        help='parse from turing machine')
    ...
    if args.from_zip is not None:
        data = load_from_zip(args.from_zip, pda=args.pda, turin=args.tm)
    else:
        data = load_from_list(args.from_files, pda=args.pda, turin=args.tm)
    if args.json:
            ...
            for edge in sorted(value['edges'],
                ...
                if 'write' in edge:
                    if edge['write'] is not None:
                        args.output.write(", write: " + edge['write'])
                if 'move' in edge:
                    if edge['move'] is not None:
                        args.output.write(", move: " + edge['move'])
                args.output.write("}\n")
            args.output.write("\n")
